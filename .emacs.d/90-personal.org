...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (defun koek/lock-file-p (file-name)
      ...
      (string-match-p (rx line-start ".#") (file-name-nondirectory file-name)))
...
#+BEGIN_SRC js
  (function (template) {
      ...
      location.href = `org-protocol://capture?${query}`;
  }("t"));
#+END_SRC

#+CAPTION: Bladwijzer
#+BEGIN_SRC js
  javascript:(function (template) {"use strict"; function toQuery(args) {return Object.keys(args).map((key) => `${key}=${encodeURIComponent(args[key])}`).join("&");} const query = toQuery({template, url: location.href, title: document.title, body: window.getSelection()}); location.href = `org-protocol://capture?${query}`;}("b"));
#+END_SRC

#+CAPTION: Notitie
#+BEGIN_SRC js
  javascript:(function (template) {"use strict"; function toQuery(args) {return Object.keys(args).map((key) => `${key}=${encodeURIComponent(args[key])}`).join("&");} const query = toQuery({template, url: location.href, title: document.title, body: window.getSelection()}); location.href = `org-protocol://capture?${query}`;}("n"));
#+END_SRC
  ...
  (use-package org-capture
    ...
    (setq org-capture-templates
          `(("j" "Journal" entry (file+olp+datetree "Inbox.org" "Journaal")
             "* %?"
             :empty-lines 1)
            ("n" "Note" entry (file+olp "Inbox.org" "Notities")
             ,(string-join '("* %^{Name} %^g"
                             "%a"
                             ""
                             "#+BEGIN_QUOTE"
