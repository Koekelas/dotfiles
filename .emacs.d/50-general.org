...
Complement =forward-word= and =backward-word=. Unlike =forward-to-word= and
...
#+BEGIN_SRC emacs-lisp
  ...
  move point to the ending of previous word."
    ...
    (let ((step (/ arg (abs arg))))
      ...
      (when (re-search-forward (rx word) nil 'no-error step)
        (backward-word step))))
  ...
  (bind-key "M-F" #'koek-mtn/next-word)
  (bind-key "M-B" #'koek-mtn/previous-word)
#+END_SRC
