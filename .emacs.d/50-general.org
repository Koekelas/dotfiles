...
#+BEGIN_SRC emacs-lisp
  (use-package smartparens
    ...
    (defun koek-sp/separate-sexp (_open-delimiter action context)
      "Separate just inserted sexp from next sexp.
  ACTION and CONTEXT are symbols, see `sp-pair'.  _OPEN-DELIMITER
  is ignored."
      (when (and (eq action 'insert) (eq context 'code))
        (save-excursion
          (sp-up-sexp)
          (unless (looking-at (rx (or blank ")" "]" "}" line-end)))
            (insert " ")))))
...
#+BEGIN_SRC emacs-lisp
  (use-package tern
    ...
    (add-hook
     'tern-mode-hook
     (koek-cpny/make-setup-backends '((company-tern company-keywords company-files
                                       :with company-yasnippet))))
...
#+BEGIN_SRC emacs-lisp
  (use-package json-mode
    ...
    (add-hook 'json-mode-hook
              (koek-cpny/make-setup-backends '((company-dabbrev company-files
                                                :with company-yasnippet)))))
#+END_SRC
