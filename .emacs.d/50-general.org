...
#+BEGIN_SRC emacs-lisp
  (use-package yasnippet
    ...
    :config
    ;;; Snippet functions
    (defun koek-ys/name-to-key (snippet-name)
      "Convert SNIPPET-NAME to snippet key.
  SNIPPET-NAME is a string."
      (string-match (rx (submatch-n 1 (one-or-more word))) snippet-name)
      (downcase (match-string 1 snippet-name)))
...
#+BEGIN_SRC emacs-lisp
  (defun koek-buff/bury-current (&optional arg)
    "Bury current, kill with prefix argument ARG.
  When current can't be killed, bury current."
    (interactive "P")
    (unless (and arg (kill-buffer))
      (bury-buffer)))
