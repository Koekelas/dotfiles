  ...
  (use-package recentf
    ...
    (mapc (lambda (file-name)
            (add-to-list 'recentf-exclude
                         (rx-to-string
                          `(and line-start ,(expand-file-name file-name)))))
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    :hook
    ((c-mode c++-mode erlang-mode mhtml-mode css-mode java-mode js2-mode json-mode
      python-mode)
     . eglot-ensure)
    :config
...
#+BEGIN_SRC emacs-lisp
  (use-package exwm
    :load-path ("lisp/xelb/" "lisp/exwm/")
    :when (string= (getenv "XDG_CURRENT_DESKTOP") "Emacs"))
#+END_SRC

** server - Emacs service
#+BEGIN_SRC emacs-lisp
  (use-package server
    :defer t
    :preface
    (defun koek-srvr/setup-socket ()
      "Setup socket."
      (setq server-name
            (expand-file-name "emacs/server" (getenv "XDG_RUNTIME_DIR"))))
    :config
    ;; Emacs is unaware of the socket created by systemd. When told
    ;; about it, Emacs tries to create it and fails. with-editor
    ;; requires the socket file name to be known. Setup socket after
    ;; server is started. Will be fixed in Emacs 27. See
    ;; https://debbugs.gnu.org/cgi/bugreport.cgi?bug=24218
    (add-hook 'emacs-startup-hook #'koek-srvr/setup-socket))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  When window theme is a dark theme, the variant is set to dark,
        ...
        ;; Only graphical frames have a window id. The server frame is a
        ;; terminal frame.
  ...
  component."
    ...
    ;; `color-rgb-to-hex' respects the color depth of the frame. The
    ;; color depth of the server frame is low.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun koek-font/retry-setup (frame)
    "Retry setup preferred font.
  FRAME is a frame, the last created frame."
    (koek-font/setup frame)               ; Frame isn't selected yet
    (when-let ((font (alist-get 'font default-frame-alist)))
      ;; Font is set but it's set after frame had its parameters set
      (set-frame-parameter frame 'font font)))

  (defun koek-font/setup (&optional frame)
    "Setup preferred font.
  FRAME is used internally."
    (if-let ((families (font-family-list (or frame (selected-frame)))))
        (progn
          ;; Frame is the first graphical frame. When no font is set,
          ;; none of the preferred fonts are installed.
          (remove-hook 'after-make-frame-functions #'koek-font/retry-setup)
          (when-let ((spec (seq-find (lambda (spec)
                                       (member (plist-get spec :family) families))
                                     koek-font/font-specs)))
            ;; Why does `set-frame-font' only work for the first
            ;; emacsclient frame?
            (let ((font (font-xlfd-name (apply #'font-spec spec) 'fold-wildcards)))
              (push `(font . ,font) default-frame-alist))))
      ;; Frame is a terminal frame. Only graphical frames support fonts.
      ;; The server frame is a terminal frame.
      (add-hook 'after-make-frame-functions #'koek-font/retry-setup)))
...
#+BEGIN_SRC emacs-lisp
  ...
  internally."
    ...
    (defconst koek-ml/eyebrowse
      '(:eval
        (when (and eyebrowse-mode (koek-ml/mru-window-p))
  ...
  checkers)."
    ...
    (defconst koek-ml/flymake
      '(:eval
        (when (and flymake-mode (koek-ml/mru-window-p))
          `("" koek-ml/separator
            ,(when-let ((name
    ...
    (defconst koek-ml/vc
      '(:eval
        (when (and vc-mode (koek-ml/mru-window-p))
