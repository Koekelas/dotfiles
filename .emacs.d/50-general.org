...
#+BEGIN_SRC emacs-lisp
  (use-package ediff
    ...
    ;; Ugly. When the control buffer is created, the window layout is
    ;; modified.
    (defvar koek-diff/layout
      nil
      "Last stored window layout.")

    (defun koek-diff/store-layout ()
      "Store current window layout.
  Call before the window layout is modified."
      (setq koek-diff/layout (current-window-configuration)))

    (defun koek-diff/setup-restore-layout-on-quit ()
      "Setup restore last stored window layout on quit.
  Call from the control buffer."
      (let ((layout koek-diff/layout))
    ...
    (add-hook 'ediff-before-setup-hook #'koek-diff/store-layout)
    (add-hook 'ediff-mode-hook #'koek-diff/setup-restore-layout-on-quit)
           ...
           (personal
            (mapcar
             ...
             (map-let
                 ...
                 (seq-reduce (pcase-lambda (rgbs `(,name . ,hex))
                               (puthash name (color-name-to-rgb hex) rgbs)
                               rgbs)
                             theme (make-hash-table))
