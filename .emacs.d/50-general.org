...
#+BEGIN_SRC emacs-lisp
  (define-advice pop-to-mark-command (:around (f) koek-mark/ensure-move)
    (let ((start (point))
          (n (length mark-ring))) ; mark-ring doesn't contain the last popped mark
      (funcall f)
...
When the last command was pop mark, press ~C-SPC~ to repeat it.
...
#+BEGIN_SRC emacs-lisp
  ...
  Unless current major mode is a member of
  `koek-af/excluded-modes', enable `auto-fill-mode'."
...
** Complete text
When the line is indented, press ~TAB~ to complete the text before
point.
...
** Minibuffer
Enable minibuffer commands (e.g. [[*counsel - ivy powered commands][counsel]]) in the minibuffer.
...
#+BEGIN_SRC emacs-lisp
  ...
  A pair is a cons. Its car is a major mode symbol, its cdr is a
  list of checker symbols.")
       ...
       (let* ((diff-n (1+ ediff-current-difference))
         ...
         `((,(max 0 (- (window-total-width) (string-width right)))
            (" "
             ...
             koek-ml/separator))
           ,right))))
    "List of mode line constructs for the ediff control buffer.")
