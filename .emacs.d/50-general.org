...
#+BEGIN_SRC emacs-lisp
  ...
  (defun koek-ml/pluralize (word n)
    "Pluralize singular WORD to match N.
  WORD is a string. N is an integer."
    (if (= n 1)
        word
      (concat word "s")))
  ...
  (defconst koek-ml/ediff
    '(" "
      ...
      (:eval
       (let ((current-diff ediff-current-difference)
             (n-diffs ediff-number-of-differences))
         (cond
          ((< current-diff 0)
           (format "Start, %d %s"
                   n-diffs (koek-ml/pluralize "diff" n-diffs)))
          ((>= current-diff n-diffs)
           (format "End, %d %s"
                   n-diffs (koek-ml/pluralize "diff" n-diffs)))
          (t
           (format "Diff %d of %d" (1+ current-diff) n-diffs))))))
