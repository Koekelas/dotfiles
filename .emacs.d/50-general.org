...
#+BEGIN_SRC emacs-lisp
  (define-advice pop-to-mark-command (:around (f) koek-mark/ensure-move)
    (let ((start (point))
          (n (length mark-ring)))
      ;; Try current mark and previous marks in mark ring
      (funcall f)
...
#+BEGIN_SRC emacs-lisp
  (use-package subword
    :hook ((prog-mode comint-mode) . subword-mode)
    :delight)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package smartparens
    ...
    (:map smartparens-mode-map
     ...
     ("C-M-k" . sp-kill-sexp))
    :hook
    (((prog-mode comint-mode) . smartparens-mode)
...
#+BEGIN_SRC emacs-lisp
  ...
  Baseline right of previous character is composed with baseline
  left of next character."
  ...
  SYMBOLS is a list of pretty symbol pairs and/or pretty symbols.
  ...
  Pretty symbol is a key in `koek-ps/comp-specs'. Its the symbol to
  replace and the symbol to replace it with."
    (let ((specs
           (seq-reduce
            (lambda (specs symbol)
              (unless (consp symbol)
...
#+BEGIN_SRC emacs-lisp
  ...
  ignored."
    ...
    (defun koek-cpny/setup-dabbrev-backend ()
      "Setup dabbrev backend for current."
...
#+BEGIN_SRC emacs-lisp
  (use-package company-tern
    ...
    :after tern
    :preface
    (defun koek-cpny/setup-tern-backend ()
      "Setup tern backend for current."
      (setq-local company-backends
...
#+BEGIN_SRC emacs-lisp
  (use-package lsp-mode
    ...
    (declare-function lsp--stdio-send-async "lsp-send")
    (declare-function lsp--stdio-send-sync "lsp-send")
...
#+BEGIN_SRC emacs-lisp
  (use-package undo-tree
    ...
    (:map undo-tree-map
     ("M-ù"   . undo-tree-redo)
     ("C-M-ù" . undo-tree-visualize))
    :init
    (bind-key "C-ù" #'undo)
    :config
    ;; FIXME: When revisiting then modifying file, history is discarded
...
#+BEGIN_SRC emacs-lisp
  (use-package tern
    ...
    (add-hook 'tern-mode-hook #'koek-cpny/setup-tern-backend)
    (add-hook 'tern-mode-hook #'koek-xref/setup-js2-backend)
    :delight)
#+END_SRC

**** Getting started
Create new file ~.tern-project~ in project home directory. Visit
~.tern-project~. Expand snippet ~browser~ or ~node~.
...
#+BEGIN_SRC emacs-lisp
  (use-package json-mode
    ...
    :config
    (add-hook 'json-mode-hook #'koek-cpny/setup-dabbrev-backend))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  zero units of COLOR2."
    ...
    (let* ((theme (alist-get 'eighties color-theme-sanityinc-tomorrow-colors))
           ...
           (specs
            (map-let
                (foreground selection lc-fg lc-bg
